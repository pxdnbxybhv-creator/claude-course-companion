...
const PAINT: Record<string, (p: P) => void> = {
  scholar(p) {
    ...
    p.fill('#c9ad78', [[62, 58], [83, 60], [84, 98], [63, 98]], 0.9, 0.4);
    for (let x = 65; x < 83; x += 3.2) p.line([[x, 60], [x + 0.5, 97]], 0.35, 0.35);
    p.brush([[57, 55, 1.4], [72, 50, 2], [89, 56, 1.2]], 0.8, '#6b4a2a');
    p.dot(68, 55, 5, 0.9, '#efe6d0', false); p.dot(76, 54, 4.5, 0.9, '#b9c6c9', false);
    bust(p, { skin: '#f2dcc3', robe: '#ebe9e0', trim: '#4b5a6b', hair: INK, blush: 0.25 });
    p.brush([[40, 33, 0.6], [50, 34, 1], [60, 32, 0.6]], 0.9, PIGMENTS.malachite, true);
    // hair ribbons
    p.brush([[46, 37, 2.2], [40, 44, 2], [33, 50, 1.4], [30, 58, 0.5]], 0.75, '#4b5a6b');
    p.brush([[53, 36, 1.8], [58, 30, 1.4], [64, 29, 0.5]], 0.6, '#4b5a6b');
  },
  gardener(p) {
    bust(p, { skin: '#e9c7a3', robe: '#5f7488', trim: '#3c4a58', hair: '#2a2420', bun: false, blush: 0.32 });
    p.fill('#dccfae', [[41, 78], [59, 78], [60, 124], [40, 124]], 0.95, 0.4);
    p.line([[41, 78], [40, 124]], 0.6, 0.5); p.line([[59, 78], [60, 124]], 0.6, 0.5);
    p.brush([[46, 60, 0.6], [50, 61.5, 0.9], [54, 60, 0.6]], 0.8, undefined, true);
    conical(p, 50, 43, 33, 16, '#d8bb78');
    p.fill('#6f8a6a', [[38, 42.5], [39, 38], [61, 38], [62, 42.5]], 0.85, 0.3);
    // the watering can
    p.fill('#b07a45', ell(78, 104, 10, 9), 0.92, 0.4);
    p.brush([[86, 104, 2.2], [93, 96, 1.8], [97, 90, 1.4]], 0.9, '#b07a45');
  ...
  fisher(p) {
    ...
    bust(p, { skin: '#e2bf9c', robe: '#9c8b5e', trim: '#6e6243', hair: '#e9e6de', eyes: 'old', blush: 0.18, bun: false });
  ...
  musician(p) {
    ...
    bust(p, { skin: '#f6e2d0', robe: '#f1e8e5', trim: '#5f7188', hair: INK, eyes: 'lady', blush: 0.28, sw: 26 });
    p.fill(INK, ell(50, 30, 8, 6), 0.95, 0.4);
    p.brush(arcW(52, 22, 6, 6, 0, Math.PI * 2, 2.6, 2.6, 16), 0.95);
    p.dot(40, 38, 4, 0.9, '#d98f98');
    // 步摇
  ...
  swordsman(p) {
    ...
    p.brush([[58, 92, 2.6], [74, 60, 2.6], [82, 44, 2.2]], 0.95, '#221e1c');
    p.brush([[73, 57, 1.2], [80, 62, 1.2]], 0.95, '#b08a4a');
    p.brush([[82, 44, 1.4], [84, 48, 1.2], [83, 54, 0.4]], 0.9, PIGMENTS.cinnabar);
    bust(p, { skin: '#ecd0b3', robe: '#343a44', trim: '#15171a', hair: INK, eyes: 'fierce', blush: 0.1, bun: false });
  ...
  taoist(p) {
    ...
    bust(p, { skin: '#f7e0c9', robe: '#a3b4be', trim: '#2c323c', hair: INK, blush: 0.4, hr: 15, hy: 56, sw: 25, bun: false });
    for (const sx of [-1, 1]) {
  ...
  painter(p) {
    ...
    bust(p, { skin: '#f0d7bd', robe: '#ddd3bb', trim: '#6b5a44', hair: INK, blush: 0.22, bun: false });
  ...
  player(p) {
    ...
    bust(p, { skin: '#f0d6bb', robe: '#8e9ba4', trim: '#2f3439', hair: INK, blush: 0.16, bun: false });
    // moustache, goatee
    ...
    // 纶巾
    p.fill('#2f3439', [[36, 46], [37, 34], [42, 28], [58, 28], [63, 34], [64, 46]], 0.95, 0.4);
    // the fan
  ...
  poet(p) {
    ...
    bust(p, { skin: '#f1d4ba', robe: '#f1f1ec', trim: '#7a96ab', hair: INK, eyes: 'smile', blush: 0.45, bun: false });
    p.fill(INK, ell(50, 35, 6, 7), 0.95, 0.4);
    p.brush([[46, 38, 2], [38, 44, 2.2], [30, 54, 1.6], [26, 66, 0.4]], 0.8, '#7a96ab');
  ...
  guan(p) {
    ...
    p.dot(14, 26, 6, 0.95, '#3f6b54', false);
    p.brush([[16, 30, 1.8], [19, 36, 1.4], [18, 42, 0.4]], 0.9, PIGMENTS.cinnabar);
    bust(p, { skin: '#b5493a', robe: '#3f6b54', trim: '#b8913f', hair: INK, eyes: 'fierce', blush: 0, sw: 36, hairCap: false });
    // pauldrons
    for (const sx of [-1, 1]) p.fill('#b8913f', ell(50 + 30 * sx, 78, 11, 6, 0, Math.PI * 2, 16), 0.9, 0.4);
    // 巾帻 and its gold band
    p.fill('#3f6b54', [...ell(50, 51, 14.5, 15.5, Math.PI * 1.02, Math.PI * 1.98, 14), [63, 46], [37, 46]], 0.95, 0.4);
    p.dot(50, 34, 9, 0.95, '#3f6b54', false);
    p.brush([[40, 44, 1.6], [50, 42.5, 2], [60, 44, 1.6]], 0.95, '#b8913f');
    // the long beard
  ...
  change(p) {
    ...
    p.brush([[8, 112, 1.2], [14, 94, 3], [24, 78, 3.6], [38, 72, 3], [50, 71, 2.6], [64, 73, 3], [78, 80, 3.6], [88, 70, 3], [94, 54, 1.2]], 0.55, '#9cc2bb');
    // long hair
    p.fill(INK, [[38, 48], [62, 48], [64, 76], [58, 92], [42, 92], [36, 76]], 0.9, 0.5);
    bust(p, { skin: '#f8eae0', robe: '#f6f2ee', trim: '#a9bccd', hair: INK, eyes: 'lady', blush: 0.24, sw: 25, bun: false });
    // 飞仙髻: two tall loops
    ...
    p.dot(50, 33, 3.4, 0.8, '#e8c3cb');
  },
  cat(p) {
    const G = '#d99550', D = '#b56a2e', C = '#f3e6cf';
    // tail curling round
