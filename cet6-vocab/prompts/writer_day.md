...
3. 写 Scene 2 情景剧：`intro` 是这一集的开场（≤ 200 字），词按出场顺序排卡片；每 3–6 张卡片之间放一个 `beat`（剧情节拍，40–220 字）。**每个节拍是一个事件**：发现一样东西、被拦住一次、或某人说了一句不该说的话；纯搬运（拧阀门、抬桌子）不算。节拍里写进接下来几张卡片中 **2–3 个**英文词就够，其余由例句承接。情景剧至少 3 个节拍。
...
- 笑点有上限也有下限：一天 47 张卡片，卡片与灰框合计 **5–18 个**标记笑点（`joke: true`），对话里另有 ≥ 2 句真笑点。story 型钩子给画面或反差，不给「先看 X 再看 Y」的操作说明。
- 例句开头去重：同一天以 The article / The report / The exhibition 一类开头的例句不超过 3 个。
- 故事完形建议加乱序词库、每词一次，避免出现两个都说得通的答案。
...
4. **人物张力不能掉线**：故事里有秘密的人（Part I 是周管理员），每天至少一处「有事没说」——一句没接的话、一次看钟、一份他先翻过的文件。不揭底，但要在场。
